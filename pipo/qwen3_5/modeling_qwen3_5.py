...
class Qwen3_5GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        if use_precomputed_states:
            # Single-token incremental decode: update conv state in-place
            ...
            # Multi-token decode: process each token through the fast incremental
            # conv1d update (same kernel as single-token decode).
            conv_outputs = []
            for t in range(seq_len):
                single = mixed_qkv[:, :, t:t+1]
                single = self.causal_conv1d_update(
                    single, conv_state,
                    self.conv1d.weight.squeeze(1), self.conv1d.bias, self.activation,
                )
                conv_outputs.append(single)
            mixed_qkv = torch.cat(conv_outputs, dim=-1)
        else:
            # Fresh prefill (no previous state)
            if cache_params is not None:
        ...
        if use_precomputed_states or has_previous_state:
            # Single- or multi-token decode with existing state: use the fast
            # fused recurrent kernel (processes tokens sequentially, no chunk overhead).
            ...
            # Fresh prefill (no previous state): use chunk mode for long sequences.
...
class Qwen3_5Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # FlashAttention-2/3 + this module (gated Q, partial RoPE head layout, GQA, mRoPE cos/sin) is unstable:
        # illegal CUDA accesses in FA often surface asynchronously in later linear-attention ops. SDPA matches the
        # known-good path; linear_attention layers still use the torch gated-delta implementation.
        # if impl is not None and "flash_attention" in impl:
        #     logger.warning_once(
        #         "Using `sdpa` for Qwen3.5 `full_attention` layers instead of `%s` to avoid FlashAttention "
        #         "illegal memory errors with this architecture; linear-attention layers are unchanged. "
        #         "Set `attn_implementation='sdpa'` on the config to silence this message.",
        #         impl,
        #     )
        #     attention_interface = ALL_ATTENTION_FUNCTIONS.get_interface("sdpa", eager_attention_forward)
        # else:
        #     attention_interface = ALL_ATTENTION_FUNCTIONS.get_interface(impl, eager_attention_forward)
        # NOW FIXED
        attention_interface = ALL_ATTENTION_FUNCTIONS.get_interface(impl, eager_attention_forward)

        # Never forward mRoPE / collator position tensors into the attention backend (varlen / packed heuristics).
        _attn_kw_drop = {
            "position_ids",
            "text_position_ids",
            "cu_seq_lens_q",
        ...
        attn_kwargs = {k: v for k, v in kwargs.items() if k not in _attn_kw_drop}
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.layer_type == "linear_attention":
            ...
            # Self Attention — do not pass `position_ids` into Qwen3_5Attention.
            # mRoPE uses shape (3, batch, seq); FlashAttention's _is_packed_sequence assumes
            # (batch, seq). With batch_size==1 that mis-detects "packed" sequences and runs
            # flash_attn_varlen with garbage cu_seqlens (illegal device memory). RoPE is
            # already applied via position_embeddings.
