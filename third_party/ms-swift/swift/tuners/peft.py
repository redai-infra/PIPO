...
class LoraConfig(peft.LoraConfig):
    ...
    # PIPO-specific fields
    compressor_type: Optional[str] = field(
        default=None, metadata={'help': 'The compressor type for PIPO models (e.g., "linear", "gated", "mlp")'})
    ...
    def save_pretrained(self, save_directory: str, **kwargs) -> None:
        ...
        # Add compressor_type if set, otherwise try to get from environment
        compressor_type = self.compressor_type
        if compressor_type is None:
            compressor_type = os.environ.get('COMPRESSOR_TYPE')
        if compressor_type is not None:
            additional_args['compressor_type'] = compressor_type
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, subfolder: Optional[str] = None, **kwargs):
        ...
        if type(self) == peft.LoraConfig:
            # Preserve custom fields when converting from peft.LoraConfig
            custom_fields = {}
            ...
            self = LoraConfig(**self.to_dict(), **custom_fields)
        elif os.path.isfile(os.path.join(pretrained_model_name_or_path, 'additional_config.json')):
            # LoraConfig loading - read additional fields
            with open(
                    os.path.join(pretrained_model_name_or_path, 'additional_config.json'), 'r', encoding='utf-8') as f:
                _json = json.load(f)
