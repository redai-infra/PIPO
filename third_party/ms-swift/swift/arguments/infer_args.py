...
class SglangArguments:
    ...
    # ----------------------------------------------------------------------
    # PIPO-specific SGLang flags (added for OPD; see
    # ``.agents/diary/2026-04-29-OPD-plan.md`` Phase 3.1).
    # When ``sglang_enable_pipo=True``, ``disable_radix_cache`` is forced
    # True (the radix cache is incompatible with the 2-token-per-step decoder
    # introduced by PIPO).
    # ----------------------------------------------------------------------
    ...
    def get_sglang_engine_kwargs(self):
        ...
        # Auto-derive disable_radix_cache when PIPO is enabled.
        disable_radix_cache = self.sglang_disable_radix_cache or self.sglang_enable_pipo

        # PIPO / radix-cache toggles are passed through ``engine_kwargs`` (extra
        # kwargs forwarded to ``sgl.Engine``'s ``ServerArgs``) because they are not
        # part of the ``SglangEngine`` constructor's named parameter list.
        engine_kwargs = {}
        if self.sglang_enable_pipo:
            ...
            if self.sglang_pipo_conf_threshold is not None and \
                ...
                engine_kwargs['pipo_conf_threshold'] = float(
