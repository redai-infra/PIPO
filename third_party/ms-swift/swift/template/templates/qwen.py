...
class Qwen3_5Template(Qwen3VLTemplate):
    ...
    def _get_position_ids(self, inputs: Dict[str, Any]):
        """Text-only Qwen3.5 (e.g. ``Qwen3_5ForCausalLM``, PIPO) uses ``Qwen3_5TextModel`` without ``get_rope_index``."""
        base_model = self.get_base_model(self._get_model())
        if not hasattr(base_model, 'get_rope_index') and not (
                hasattr(base_model, 'model') and hasattr(base_model.model, 'get_rope_index')):
            attention_mask = inputs.get('attention_mask_2d')
            if attention_mask is None:
                attention_mask = inputs.get('attention_mask')
            input_ids = inputs['input_ids']
            ...
            pos = (mask.cumsum(-1) - 1).clamp(min=0) * mask
            position_ids_3d = pos.unsqueeze(0).expand(3, -1, -1).to(dtype=input_ids.dtype)
            return self._concat_text_position_ids(position_ids_3d)
        return Qwen2VLTemplate._get_position_ids(self, inputs)
