...
class Template(ProcessorMixin):
    ...
    def __init__(
        ...
        *,
        ...
        min_length: Min length of the sequence
        ...
        self.task_type: Literal['causal_lm', 'pipo', 'seq_cls', 'embedding', 'prm', 'reranker',
    ...
    def encode(self,
        ...
        if isinstance(inputs, dict):
            if self.task_type in ('causal_lm', 'pipo') and not self.is_training:
        ...
        chosen = inputs.chosen
        if self.task_type in ('causal_lm', 'pipo'):
            ...
            if i < n_round - 1:
                ...
                if add_eos is None:
                    add_eos = (self.is_training
                               or self.task_type not in ('causal_lm', 'pipo')) and not sep_token and not endswith_stop_words
                if add_eos:
    ...
    def _encode_truncated(self, inputs: StdTemplateInputs):
        ...
        # Check min_length constraint first
        if self.min_length is not None and length < self.min_length:
            raise MinLengthError(f'Current length of row({length}) is smaller'
                                 f' than the min_length({self.min_length}).')
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        if (self.template_meta.template_type == 'dummy' and self.use_chat_template and not self.is_training
                and self.task_type in ('causal_lm', 'pipo')):
