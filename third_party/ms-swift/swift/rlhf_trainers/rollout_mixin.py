...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _prepare_vllm(self):
        """Initialize vLLM/SGLang engine (server or colocate mode).

        ``args.rollout_backend`` (added for PIPO × SGLang OPD; see
        ``.agents/diary/2026-04-29-OPD-plan.md``) selects between vLLM (default) and
        SGLang. SGLang only supports colocate mode here; server mode raises
        ...
        # Backend selector (vllm | sglang). Default to vllm for backward compatibility.
        self.rollout_backend = getattr(args, 'rollout_backend', 'vllm')
        self.vllm_version_ge_0_10_2 = check_vllm_version_ge('0.10.2') if self.rollout_backend == 'vllm' else False
        self.disable_rollout_importance_sampling = not self.vllm_version_ge_0_10_2
        if not args.use_vllm:
            return

        # SGLang server mode is explicitly out of scope (see OPD plan §5.6).
        if self.rollout_backend == 'sglang' and self.vllm_mode == 'server':
            raise NotImplementedError(
                'SGLang rollout currently supports colocate mode only. '
                'Use --vllm_mode colocate (or switch --rollout_backend vllm).')

        if self.rollout_backend == 'vllm':
            if not is_vllm_available():
                raise ImportError('vLLM is not available and `use_vllm` is set to True. '
                                  'Please install vLLM with `pip install vllm -U` to use it.')
            if (not self.vllm_version_ge_0_10_2
                    and getattr(self.args, 'rollout_importance_sampling_mode', None) is not None):
            ...
            try:
                ...
                raise ImportError('SGLang is not available and `rollout_backend == "sglang"`. '
                                  'Please install sglang first.') from exc
            # SGLang doesn't expose vLLM's importance-sampling correction; force-disable.
        ...
        if self.vllm_mode == 'server':
            ...
            with context():
                if self.rollout_backend == 'sglang':
                    self.engine = self._prepare_sglang_engine()
                    if args.sleep_level > 0:
                        # SGLang's equivalent of vLLM's `sleep` is release_memory_occupation.
                        self._engine_sleep(args.sleep_level)
                else:
    ...
    # ------------------------------------------------------------------
    # SGLang colocate engine (added for PIPO × SGLang OPD)
    # See ``.agents/diary/2026-04-29-OPD-plan.md`` Phase 2.3.
    # ------------------------------------------------------------------
    def _prepare_sglang_engine(self):
        """Create and configure a SGLang colocate engine for rollout.

        Mirrors :meth:`_prepare_vllm_engine` for the SGLang side. The PIPO
        trainer overrides this method to force ``enable_pipo=True`` /
        ``disable_radix_cache=True`` regardless of CLI args (see
        :class:`pipo.trainer.swift_gkd_trainer.PIPOGKDTrainer`).
        """
        from swift.infer_engine import GRPOSglangEngine
        args = self.args
        model = self.model

        sglang_template = copy(self.template)
        sglang_template.padding_free = False
        sglang_template.sequence_parallel_size = 1

        # All SGLang engine knobs come from the rollout-side mixin (NOT the inference
        # SglangArguments class), so we don't pull in the inference args here.
        sglang_kwargs = args.get_rollout_sglang_engine_kwargs()
        engine_kwargs = sglang_kwargs.pop('engine_kwargs', None) or {}
        # OPD rollout passes prompt strings to SGLang Engine, and PIPO may need
        # pad_token_id during tokenizer-side pair padding, so tokenizer init is required.
        engine_kwargs.setdefault('skip_tokenizer_init', False)
        # In torchrun colocate mode, every training rank creates its own SGLang
        # engine. Bind that engine to the rank-local GPU; otherwise all engines may
        # default to GPU 0 and over-subscribe the same device.
        engine_kwargs.setdefault('base_gpu_id', self.accelerator.local_process_index)

        with Swift.grpo_context(model, self.template.processor):
            set_expandable_segments(False)
            engine = GRPOSglangEngine(
                model.model_dir,
                torch_dtype=model.model_info.torch_dtype,
                model_type=model.model_meta.model_type,
                template=sglang_template,
                engine_kwargs=engine_kwargs,
                **sglang_kwargs,
            )
            set_expandable_segments(True)

        # Track whether the engine is currently sleeping (released). SGLang doesn't
        # expose an `is_sleeping` flag like vLLM's, so we maintain it ourselves.
        engine._is_released = False
        return engine

    # ------------------------------------------------------------------
    # Backend adapter methods — abstract over vLLM vs SGLang runtime calls.
    # ------------------------------------------------------------------
    def _engine_is_sleeping(self) -> bool:
        if getattr(self, 'rollout_backend', 'vllm') == 'sglang':
            return bool(getattr(self.engine, '_is_released', False))
        return bool(self.engine.inner_model_executor.is_sleeping)

    def _engine_sleep(self, level: int = 1, tags: Optional[List[str]] = None):
        if getattr(self, 'rollout_backend', 'vllm') == 'sglang':
            # SGLang: release_memory_occupation supports `tags=['weights'/'kv_cache']`.
            release = self.engine.engine.release_memory_occupation
            try:
                if tags is not None and 'tags' in inspect.signature(release).parameters:
                    release(tags=tags)
                else:
                    release()
            except TypeError:
                release()
            self.engine._is_released = True
        else:
            self.engine.engine.sleep(level)

    def _engine_wake_up(self, tags: Optional[List[str]] = None):
        if getattr(self, 'rollout_backend', 'vllm') == 'sglang':
            resume = self.engine.engine.resume_memory_occupation
            try:
                if tags is not None and 'tags' in inspect.signature(resume).parameters:
                    resume(tags=tags)
                else:
                    resume()
            except TypeError:
                resume()
            self.engine._is_released = False
        else:
            wake_up = self.engine.engine.wake_up
            kwargs = {'tags': tags} if tags is not None and 'tags' in inspect.signature(wake_up).parameters else {}
            wake_up(**kwargs)

    def _engine_reset_cache(self):
        if getattr(self, 'rollout_backend', 'vllm') == 'sglang':
            self.engine.engine.flush_cache()
        else:
            self.engine.engine.reset_prefix_cache()

    def _engine_load_state_dict(self, state_dict):
        """Push ``state_dict`` to the colocate engine (vLLM or SGLang)."""
        if getattr(self, 'rollout_backend', 'vllm') == 'sglang':
            # SGLang takes List[Tuple[str, torch.Tensor]] and serialises tensors via
            # multiprocessing for cross-worker shipping. ``flush_cache=False`` because
            # the caller (_move_model_to_vllm) flushes once at the end.
            self.engine.engine.update_weights_from_tensor(
    ...
    def _move_adapter_to_vllm(self):
        ...
        if self.vllm_mode == 'server' and self.accelerator.is_main_process:
            ...
            if getattr(self, 'rollout_backend', 'vllm') == 'sglang':
                # LoRA hot-swap on SGLang rollout is explicitly out of scope (see
                # ``.agents/diary/2026-04-29-OPD-plan.md`` §11). The PIPO plan uses
                # full weight sync (vllm_enable_lora=False); LoRA adapter weights are
                # merged into the base before _move_full_model_to_vllm pushes them.
                raise NotImplementedError(
                    'LoRA adapter hot-swap is not supported with rollout_backend=sglang. '
                    'Set vllm_enable_lora=False so LoRA is merged before weight sync.')
    ...
    def _load_state_dict_to_vllm(self, state_dict):
        ...
        if self.vllm_mode == 'server' and self.accelerator.is_main_process:
            ...
            self._engine_load_state_dict(state_dict)
        del state_dict
    ...
    def _fast_infer(self, inputs: DataType) -> DataType:
        """Efficient inference with vLLM/SGLang colocate mode support.

        Uses the ``_engine_*`` adapters to handle both backends transparently.
        """
        args = self.args
        ...
        if self.vllm_mode == 'colocate' and args.sleep_level > 0:
            if self._engine_is_sleeping():
                ...
                # Wake up just the weights region first; KV cache is woken up below
                # right before the actual generate call. SGLang may ignore the tag
                # and wake everything in one shot — the adapter handles that.
                self._engine_wake_up(tags=['weights'])
        ...
        with context():
            ...
            if (self.vllm_mode == 'colocate' and self._engine_is_sleeping()):
                aggressive_empty_cache()
                set_expandable_segments(False)
                self._engine_wake_up(tags=['kv_cache'])
            ...
            if self.vllm_mode == 'colocate' and args.sleep_level > 0:
                self._engine_reset_cache()
                self._engine_sleep(level=args.sleep_level)
                aggressive_empty_cache()
                set_expandable_segments(True)
        return outputs
