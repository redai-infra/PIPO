...
class RolloutTrainerArgumentsMixin(VllmArguments):
    ...
    Args:
        ...
        presence_penalty (float): Penalizes tokens that already appeared in the generated text. Defaults to 0.0.
    ...
    # ----------------------------------------------------------------------
    # Rollout backend selector (added for PIPO × SGLang OPD; see
    # ``.agents/diary/2026-04-29-OPD-plan.md`` Phase 2.1).
    # ----------------------------------------------------------------------
    # Choose the inference backend used to generate on-policy rollouts during
    # GRPO/GKD training. ``vllm`` (the default) preserves existing behaviour.
    # ``sglang`` enables the SGLang colocate path — required for PIPO
    # because vLLM cannot host the 2:1 KV-cache compression ratio.
    rollout_backend: Literal['vllm', 'sglang'] = 'vllm'

    # ----------------------------------------------------------------------
    # SGLang colocate engine knobs (mirrors a subset of SglangArguments).
    # We deliberately re-declare here instead of multiple-inheriting because
    # SglangArguments lives in swift.arguments.infer_args, which already
    # depends on this mixin via VllmArguments — the import cycle would force a
    # late binding. The duplication is small and stable.
    # ----------------------------------------------------------------------
    ...
    # PIPO-specific SGLang flags (see Phase 3.1 in the plan).
    sglang_enable_pipo: bool = False
    sglang_disable_radix_cache: bool = False  # auto-True when enable_pipo
    sglang_pipo_conf_threshold: float = -1.0
    sglang_engine_kwargs: Optional[Union[dict, str]] = None

    def __post_init__(self):  # type: ignore[override]
        # Allow VllmArguments.__post_init__ to run if defined.
        if hasattr(super(), '__post_init__'):
            super().__post_init__()
        # JSON string -> dict
        if isinstance(self.sglang_engine_kwargs, str) and self.sglang_engine_kwargs:
            self.sglang_engine_kwargs = json_parse_to_dict(self.sglang_engine_kwargs)
        elif self.sglang_engine_kwargs is None:
            self.sglang_engine_kwargs = {}
        # PIPO forces disable_radix_cache=True (radix cache is incompatible
        # with the 2-token-per-step decoder). The user can still override via
        # ``--sglang_disable_radix_cache true`` explicitly.
        if self.sglang_enable_pipo and not self.sglang_disable_radix_cache:
            self.sglang_disable_radix_cache = True

    def get_rollout_sglang_engine_kwargs(self) -> Dict[str, Any]:
        """Build the kwargs dict passed to GRPOSglangEngine for colocate mode.

        Centralised here (rather than on SglangArguments) so the rollout-mixin
        does not need to import the inference-side argument class.
        """
        kwargs: Dict[str, Any] = {
        ...
        # extra engine kwargs (passed through to sgl.Engine via ServerArgs)
        extra: Dict[str, Any] = dict(self.sglang_engine_kwargs or {})
        if self.sglang_enable_pipo:
            extra['enable_pipo'] = True
            extra['disable_radix_cache'] = True
            if self.sglang_pipo_conf_threshold is not None and \
                    self.sglang_pipo_conf_threshold >= 0:
                extra['pipo_conf_threshold'] = float(
                    self.sglang_pipo_conf_threshold)
        kwargs['engine_kwargs'] = extra
        return kwargs
