...
@contextmanager
def _clean_torchelastic_env_for_sglang():
    """Avoid leaking torchrun/elastic rendezvous env into SGLang subprocesses.

    When SGLang is launched from a torchrun training worker, child scheduler
    processes inherit ``TORCHELASTIC_USE_AGENT_STORE=True``. PyTorch's
    ``tcp://`` rendezvous then treats even rank 0 as a client of the elastic agent
    store instead of creating a TCPStore server, so SGLang's internal
    ``init_process_group(rank=0, world_size=1)`` hangs connecting to its own port.
    """
    keys = ['TORCHELASTIC_USE_AGENT_STORE']
...
class SglangEngine(InferEngine):
    ...
    def __init__(
        ...
        *,
        ...
        with _clean_torchelastic_env_for_sglang():
    ...
    def _prepare_server_args(self, engine_kwargs):
        ...
        # SGLang internally reserves one extra token position (e.g. for the EOS
        # scratchpad or scheduler bookkeeping), so we deduct 1 from the user-facing
        # limit to avoid exceeding the KV-cache budget.
        ...
        skip_tokenizer_init = engine_kwargs.pop('skip_tokenizer_init', True)
    ...
    async def infer_async(self,
        ...
        # NOTE: Removed ``with torch.inference_mode():`` wrap that previously
        # surrounded this ``await``. The wrap was REDUNDANT (``template.encode``
        # is already decorated with ``@torch.inference_mode()`` so the encode
        # body runs in inference_mode on the worker thread regardless), and
        # CONCURRENTLY HARMFUL: when ``_batch_infer_stream`` runs N>1
        # ``infer_async`` coroutines via ``asyncio.gather``, the multiple
        # ``with`` blocks push ``InferenceMode`` RAII guards onto the SAME
        # thread-local stack on the event-loop thread. Coroutines suspend at
        # ``await`` and resume in arbitrary order, so the guards are destructed
        # OUT OF ORDER. Each guard's destructor restores the state captured at
        # construction (``prev_state``); out-of-order destruction can leave
        # the thread permanently in inference_mode (the LAST destructed guard
        # had captured prev=True from a sibling that pushed on top of it).
        #
        # Symptom downstream (PIPO OPD with B>1):
        #   * HF Trainer's ``training_step`` runs ``compute_loss`` with
        #     ``is_grad_enabled() == False``.
        #   * Tensors materialised after ``_fast_infer`` returns become
        #     "inference tensors" that cannot be saved for backward.
        #   * ``accelerator.backward(loss)`` raises
        #     ``RuntimeError: element 0 of tensors does not require grad``.
        #
        # Removing the wrap here is sufficient because ``template.encode`` runs
        # in the worker thread under its own ``@torch.inference_mode()``, which
        # has its own thread-local guard stack and does NOT interleave across
        # event-loop coroutines.
        #
        # See ``tests/test_asyncio_inference_mode_leak.py`` for the minimal
        # reproducer and ``pipo/trainer/swift_gkd_trainer.py::training_step``
        # for the in-trainer defense layer.
