...
class Qwen3_5ForCausalLMLatent(nn.Module):
    ...
    def __init__(
        ...
        # Decoder layers
        # SGLang's Qwen3NextConfig provides layers_block_type as a @property
        # (uses "attention" / "linear_attention"). HuggingFace's Qwen3_5TextConfig
        # stores layer_types in config.json (uses "full_attention" / "linear_attention").
        _block_types = getattr(config, "layers_block_type", None) or getattr(config, "layer_types", None)

        # MTP block may set _layer_id_offset so its single layer gets a
        # unique layer_id outside the backbone range (e.g. 32 for a 32-layer
        # backbone).  This avoids KV cache collisions and ensures the
        # HybridLinearAttnBackend routes it to full-attention.
        _layer_id_offset = getattr(config, "_layer_id_offset", 0)

        def get_layer(idx: int, prefix: str):
            layer_type = _block_types[idx]
...
class Qwen3_5MoeForCausalLMLatent(Qwen3_5ForCausalLMLatent):
    def __init__(
        self,
        config: Qwen3_5TextConfig,
        quant_config: Optional[QuantizationConfig] = None,
...
class Qwen3_5ForConditionalGenerationLatent(Qwen3VLForConditionalGeneration):
    def __init__(
        self,
        config: Qwen3_5Config,
        quant_config: Optional[QuantizationConfig] = None,
        prefix: str = "",
        language_model_cls=Qwen3_5ForCausalLMLatent,
    ):
...
class Qwen3_5MoeForConditionalGenerationLatent(Qwen3VLForConditionalGeneration):
    ...
    def __init__(
        ...
        quant_config: Optional[QuantizationConfig] = None,
        prefix: str = "",
        language_model_cls=Qwen3_5MoeForCausalLMLatent,
    ) -> None:
        super().__init__(config, quant_config, prefix, language_model_cls)
        rope_config = getattr(self.config, "rope_parameters", None) or getattr(
            self.config, "rope_scaling", {}
...
EntryClass = [Qwen3_5MoeForConditionalGenerationLatent, Qwen3_5ForConditionalGenerationLatent]
