...
class SchedulerOutputProcessorMixin:
    ...
    def process_batch_result_prefill(
        ...
        if self.is_generation:
            ...
            for i, (req, next_token_id) in enumerate(zip(batch.reqs, next_token_ids)):
                ...
                if req.is_chunked <= 0:
                    ...
                    # For PIPO, next_token_id may be a list [tok1, tok2].
        ...
        # ── PIPO: NO compaction needed ──
        # With the new design, the scheduler pads origin_input_ids to even
        # length and allocates KV slots at compressed (L//2) granularity from
        # the start.  The backbone writes to all allocated slots directly.
        # No post-prefill compaction, no stride-2 subsampling, no -1 sentinels.
    ...
    def process_batch_result_decode(
        ...
        if batch.spec_algorithm.is_none():
            ...
            enable_multi_token_decode = (
                server_args.enable_pipo
            )
            next_token_ids = next_token_ids.tolist()
            if batch.return_logprob and not enable_multi_token_decode:
                # Logprob for multi-token decode is not yet supported
                # (phase1+phase2 merge needed).
                next_token_logprobs = logits_output.next_token_logprobs.tolist()
        elif batch.is_spec_v2:
            next_token_ids = self._resolve_spec_overlap_token_ids(result, batch)
            tokens_per_req = None  # varies per req

        def _token_count(ids):
            return len(ids) if isinstance(ids, (list, tuple)) else 1

        self.num_generated_tokens += (
            sum(_token_count(ids) for ids in next_token_ids)
            if batch.spec_algorithm.is_none()
            else len(batch.reqs)
        )
        ...
        for i, (req, next_token_id) in enumerate(zip(batch.reqs, next_token_ids)):
            ...
            if (
                ...
                and not enable_multi_token_decode
            ):
                # speculative worker handles logprob in speculative decoding;
                # multi-token decode does not yet merge phase1/phase2 logprobs.
            ...
            if req.grammar is not None:
                ...
                try:
                    ...
                    for token_id in tokens_to_accept:
