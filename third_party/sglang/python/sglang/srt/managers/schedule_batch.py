...
from sglang.srt.mem_cache.utils import convert_to_pair_key
...
class Req(ReqDllmMixin):
    ...
    def init_next_round_input(
        ...
        if enable_pipo:
            # For PIPO, the minimum cacheable unit is a token pair (2 tokens).
            # We need at least 1 pair for forward, so max matchable pairs = n_pairs - 1.
            n_pairs = input_len // 2
            max_prefix_pairs = max(n_pairs - 1, 0)
            # Convert fill_ids to pair-keys for prefix matching
            token_ids_for_match = convert_to_pair_key(
                self.fill_ids[: max_prefix_pairs * 2]
            )
        else:
            ...
            token_ids_for_match = self.fill_ids[:max_prefix_len]
        ...
        if tree_cache is not None:
            ...
            match_result = tree_cache.match_prefix(
                MatchPrefixParams(
                    key=RadixKey(
                        token_ids=token_ids_for_match, extra_key=self.extra_key
                    ),
                    req=self,
                    cow_mamba=cow_mamba,
                )
        ...
        # PIPO: prefix_indices is at compressed (pair-key) granularity,
        # fill_ids is at uncompressed token granularity.
        if enable_pipo:
            # Convert prefix_indices to uncompressed length
            prefix_len_uncomp = len(self.prefix_indices) * 2
            # Clamp: cannot exceed fill_ids length (e.g., after padding removal)
            prefix_len_uncomp = min(prefix_len_uncomp, len(self.fill_ids))
            extend_len_uncomp = len(self.fill_ids) - prefix_len_uncomp
            # Store compressed extend_len for consistency with schedule_policy
            self.set_extend_input_len(extend_len_uncomp // 2)
        else:
    ...
    def _check_token_based_finish(self, new_accepted_tokens: List[int]) -> bool:
        ...
        for i, token_id in enumerate(new_accepted_tokens):
            # PIPO: PAD token is a valid input token and should not trigger stop
            if (
                get_global_server_args().enable_pipo
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    input_ids_pair: Optional[torch.Tensor] = None  # [batch_size, 2] for PIPO decode
    ...
    def prepare_for_extend(self):
        ...
        # ── PIPO: pad and compute at compressed granularity ──
        # The compressor reshapes [T, H] → [T//2, 2*H], so T must be even.
        # For PIPO, prefix_indices has compressed (KV) length after
        # cache_unfinished_req, while fill_ids is at uncompressed length.
        # We must account for this mismatch when computing extend lengths.
        if enable_pipo:
            for r in reqs:
                # Determine how many uncompressed tokens are already processed.
                # prefix_indices length is in compressed (KV) granularity for
                # chunked-prefill continuation, so multiply by 2 to get the
                # uncompressed offset into fill_ids.
                pre_len_uncomp = len(r.prefix_indices) * 2
                # Clamp: if fill_ids was truncated shorter, use fill_ids length
                pre_len_uncomp = min(pre_len_uncomp, len(r.fill_ids))
                # The uncompressed extend portion that still needs processing
                extend_uncomp = len(r.fill_ids) - pre_len_uncomp
                # Note: Both pre_len_uncomp and extend_uncomp should be even because:
                # 1. Tokenizer ensures original sequence length is even
                # 2. schedule_policy ensures trunc_len is even (chunked prefill)
                # Therefore, no padding is needed here.
                r._latent_pre_len_uncomp = pre_len_uncomp
                r._latent_extend_uncomp = extend_uncomp

        if enable_pipo:
            # Build everything in uncompressed space first, then halve
            input_ids = [r.fill_ids[r._latent_pre_len_uncomp:] for r in reqs]
            seq_lens = [len(r.fill_ids) for r in reqs]
            orig_seq_lens = [max(len(r.fill_ids), len(r.origin_input_ids)) for r in reqs]
            extend_lens = [r._latent_extend_uncomp for r in reqs]
            extend_num_tokens = sum(extend_lens)
            prefix_lens = [len(r.prefix_indices) for r in reqs]  # compressed prefix

            # Save uncompressed padded data for the model
            self._pipo_padded_input_ids_list = [ids[:] for ids in input_ids]
            self._pipo_padded_extend_lens = extend_lens[:]  # uncompressed, all even

            # Halve to compressed granularity
            seq_lens = [s // 2 for s in seq_lens]
            orig_seq_lens = [s // 2 for s in orig_seq_lens]
            # prefix_lens already in compressed granularity — don't halve
            extend_lens = [e // 2 for e in extend_lens]
            extend_num_tokens = sum(extend_lens)
            # Truncate input_ids to compressed length (dummy IDs for allocator)
            input_ids = [ids[:elen] for ids, elen in zip(input_ids, extend_lens)]
            # Update req fields for consistency downstream
            for r in reqs:
                r.extend_input_len = r._latent_extend_uncomp // 2
        else:
        ...
        # ── PIPO: store full padded input_ids for embedding ──
        if enable_pipo:
            padded_flat = list(chain.from_iterable(self._pipo_padded_input_ids_list))
            self.pipo_padded_input_ids = torch.tensor(
                padded_flat, dtype=torch.int64, pin_memory=_pin
            ).to(self.device, non_blocking=True)
            self.pipo_padded_extend_lens = self._pipo_padded_extend_lens
        else:
            self.pipo_padded_input_ids = None
            self.pipo_padded_extend_lens = None
        self.input_embeds = (
    ...
    def prepare_for_decode(self):
        ...
        # Update fields
        enable_pipo = get_global_server_args().enable_pipo
        # Backbone operates on *compressed* latent tokens (1 per pair), so
        # KV-cache allocation is always 1 slot per request per decode step.
        token_per_req = 1

        if enable_pipo:
            # Build input_ids_pair [batch_size, 2]: last 2 tokens per req.
            # After prefill, output_ids = [token1, PAD] (always 2 elements).
            # After each decode step, 2 more tokens are appended.
            # So output_ids is always >= 2, and output_ids[-2:] gives the
            # latest pair. This avoids the O(prompt_len + gen_len) cost of
            # ``origin_input_ids + output_ids`` list concatenation.
            pair_list = [r.output_ids[-2:] for r in self.reqs]
            self.input_ids_pair = torch.tensor(
                pair_list, dtype=torch.int64, device=self.device
            )
            # input_ids for PIPO decode: use the first token of each pair.
            # This has size = batch_size (one token per request), which matches
            # the CUDA graph buffer expectations for decode (raw_num_token = bs).
            self.input_ids = self.input_ids_pair[:, 0].contiguous()
    ...
    def filter_batch(
        ...
        if self.input_ids_pair is not None:
            if keep_indices_device.max().item() >= self.input_ids_pair.shape[0]:
                # input_ids_pair is stale (size doesn't match the pre-filter
                # batch size). This can happen if merge_batch added reqs
                # without updating input_ids_pair. Invalidate it —
                # prepare_for_decode will rebuild before next forward.
                logger.warning(
                    "PIPO: input_ids_pair stale "
                    f"(shape={self.input_ids_pair.shape[0]}, "
                    f"max_index={keep_indices_device.max().item()}, "
                    f"reqs={len(self.reqs)}). Clearing."
                )
                self.input_ids_pair = None
            else:
                self.input_ids_pair = self.input_ids_pair[keep_indices_device]
    ...
    def merge_batch(self, other: "ScheduleBatch"):
        ...
        # Merge input_ids_pair for PIPO
        self_input_ids_pair = getattr(self, "input_ids_pair", None)
        other_input_ids_pair = getattr(other, "input_ids_pair", None)
        if self_input_ids_pair is not None and other_input_ids_pair is not None:
            self.input_ids_pair = torch.cat([self_input_ids_pair, other_input_ids_pair])
        elif other_input_ids_pair is not None:
            # self (prefill) doesn't have input_ids_pair yet, but other (decode) does.
            # Create placeholder for prefill requests and concatenate.
            # input_ids_pair will be properly created in prepare_for_decode.
            num_prefill_reqs = len(self.reqs)
            placeholder = torch.zeros(
                (num_prefill_reqs, 2),
                dtype=other_input_ids_pair.dtype,
                device=other_input_ids_pair.device,
            )
            self.input_ids_pair = torch.cat([placeholder, other_input_ids_pair])
        elif self_input_ids_pair is not None:
            # self (decode) has input_ids_pair, but other (extend) does not.
            # Invalidate the stale tensor — prepare_for_decode will rebuild it
    ...
    def get_model_worker_batch(
        ...
        return ModelWorkerBatch(
            ...
            input_ids_pair=getattr(self, "input_ids_pair", None),
            pipo_phase=1,
            pipo_padded_input_ids=getattr(self, "pipo_padded_input_ids", None),
            pipo_padded_extend_lens=getattr(self, "pipo_padded_extend_lens", None),
        )
...
class ModelWorkerBatch:
    ...
    # For PIPO: 2 tokens per decode step
    input_ids_pair: Optional[torch.Tensor] = None  # [batch_size, 2] for decode
    pipo_phase: int = 1  # 1=backbone, 2=MTP
    # Full padded input_ids for PIPO prefill (model embeds these, then compresses)
    pipo_padded_input_ids: Optional[torch.Tensor] = None
    # Per-request padded extend lengths (original, uncompressed) for PIPO
    pipo_padded_extend_lens: Optional[List[int]] = None
