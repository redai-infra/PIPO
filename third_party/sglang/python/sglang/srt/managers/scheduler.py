...
class Scheduler(
    ...
    def __init__(
        ...
        # PIPO produces 2 tokens per decode step; the
        # overlap scheduling FutureMap cannot handle multi-token results, so
        # we forcefully disable overlap when either mode is active.
        if  server_args.enable_pipo and self.enable_overlap:
            logger.info(
                "Disabling overlap scheduling because multi-token decode "
                "--enable-pipo is set "
                "and incompatible with FutureMap."
            )
    ...
    def init_cache_with_memory_pool(self):
        ...
        params = CacheInitParams(
            ...
            is_pipo=server_args.enable_pipo,
            tp_cache_group=(
    ...
    def _get_new_batch_prefill_raw(
        ...
        # PIPO: budget is at compressed granularity (L//2)
        if chunked_prefill_size is not None and get_global_server_args().enable_pipo:
            chunked_prefill_size = chunked_prefill_size // 2
        if self.chunked_req is not None and self.enable_dynamic_chunking:
            ...
            if dynamic_size is not None:
                ...
                if get_global_server_args().enable_pipo:
                    chunked_prefill_size = dynamic_size // 2
