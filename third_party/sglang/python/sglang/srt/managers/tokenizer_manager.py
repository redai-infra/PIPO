...
class TokenizerManager(TokenizerCommunicatorMixin, TokenizerManagerMultiItemMixin):
    ...
    async def _tokenize_one_request(
        ...
        # PIPO: ensure input_ids has even length for pair compression
        if self.server_args.enable_pipo and len(input_ids) % 2 != 0:
    ...
    async def _batch_tokenize_and_process(
        ...
        for i, req in enumerate(requests):
            # PIPO: ensure input_ids has even length for pair compression
            if self.server_args.enable_pipo and len(input_ids_list[i]) % 2 != 0:
                input_ids_list[i].append(self.tokenizer.pad_token_id)
