...
class PrefillAdder:
    ...
    def add_chunked_req(self, req: Req):
        ...
        truncated = req.extend_input_len > _rem_tokens
        new_len = min(req.extend_input_len, _rem_tokens)
        req.set_extend_input_len(new_len)
        # PIPO: extend_input_len is in compressed granularity, but fill_ids is uncompressed
        # Multiply by 2 to get the correct uncompressed truncation length
        if get_global_server_args().enable_pipo:
            trunc_len_uncomp = (len(req.prefix_indices) + req.extend_input_len) * 2
        else:
            trunc_len_uncomp = len(req.prefix_indices) + req.extend_input_len
        req.fill_ids = req.fill_ids[:trunc_len_uncomp]
        self.can_run_list.append(req)
    ...
    def add_one_req_ignore_eos(self, req: Req):
        ...
        if self.dllm_config is not None:
            ...
            # PIPO: extend_input_len is in compressed granularity, but
            # fill_ids is uncompressed.  Multiply by 2 to get the correct
            # uncompressed truncation length.
            if get_global_server_args().enable_pipo:
                req.fill_ids = req.fill_ids[:trunc_len * 2]
            else:
    ...
    def add_one_req(
        ...
        with self._lock_node(req.last_node):
            ...
            if self.dllm_config is not None:
                ...
                # uncompressed truncation length.
                if get_global_server_args().enable_pipo:
                    req.fill_ids = req.fill_ids[: (len(req.prefix_indices) + req.extend_input_len) * 2]
                else:
                    req.fill_ids = req.fill_ids[: len(req.prefix_indices) + req.extend_input_len]
