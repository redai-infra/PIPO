...
def _pipo_conf_threshold() -> float:
    raw = os.environ.get("PIPO_CONF_THRESHOLD", "-1")
    try:
        value = float(raw)
    except ValueError as exc:
        raise ValueError(
            f"PIPO_CONF_THRESHOLD must be a float, got {raw!r}"
        ) from exc
    return value
...
class TpModelWorker(BaseTpWorker):
    ...
    def _resolve_pad_token_id(self) -> int:
        ...
        for cfg_name in ('hf_config', 'hf_text_config'):
            cfg = getattr(self.model_config, cfg_name, None)
            pad_token_id = getattr(cfg, 'pad_token_id', None)
            if pad_token_id is not None:
                return int(pad_token_id)
        # Qwen3.5 pad token fallback. This path is used when SGLang is embedded in
        # training with skip_tokenizer_init=True.
        return 248044
    ...
    def forward_batch_generation(
        ...
        if self.pp_group.is_last_rank:
            ...
            if not model_worker_batch.is_prefill_only:
                ...
                if enable_pipo and model_worker_batch.input_ids_pair is not None:
                    # PIPO two-phase decode: phase1 -> sample token1 -> phase2 -> sample token2
                    token1 = self.model_runner.sample(logits_output, forward_batch)

                    pipo_conf_thrshold = _pipo_conf_threshold()
                    use_conf_thr = pipo_conf_thrshold >= 0
                    skip_phase2 = pipo_conf_thrshold >= 1.0

                    if skip_phase2:
                        token2 = torch.full_like(token1, self.pad_token_id)
                        batch_result.next_token_ids = torch.stack([token1, token2], dim=1)
                    else:
                        forward_batch.pipo_phase = 2
                        forward_batch.pipo_sampled_token_from_phase1 = token1
                        out2 = self.model_runner.forward(
                            forward_batch,
                            pp_proxy_tensors=pp_proxy_tensors,
                            skip_attn_backend_init=skip_attn_backend_init,
                        )

                        token2 = self.model_runner.sample(out2.logits_output, forward_batch)
                        to_be_pad = None
                        if use_conf_thr:
                            conf = getattr(forward_batch, "pipo_token2_conf", None)
                            to_be_pad = conf < pipo_conf_thrshold
                        if to_be_pad is not None and to_be_pad.any():
                            token2[to_be_pad] = self.pad_token_id

                        batch_result.next_token_ids = torch.stack([token1, token2], dim=1)
                        batch_result.logits_output = out2.logits_output

                elif enable_pipo and forward_batch.forward_mode.is_extend():
                    # does not support MTP in prefilling currently
                    token1 = self.model_runner.sample(logits_output, forward_batch)
                    # TODO: check this: can't we really get token2 at prefill?
                    token2 = torch.full_like(token1, self.pad_token_id)
                    batch_result.next_token_ids = torch.stack([token1, token2], dim=1)
                else:
                    batch_result.next_token_ids = self.model_runner.sample(
