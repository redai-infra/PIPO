...
from sglang.srt.mem_cache.utils import convert_to_bigram_key, convert_to_pair_key
...
class RadixCache(BasePrefixCache):
    ...
    def cache_finished_req(self, req: Req, is_insert: bool = True):
        ...
        # ── PIPO: kv_committed_len is at compressed (L//2+D) granularity,
        # but token_ids are at uncompressed granularity.  Take 2× tokens and
        # convert to non-overlapping pair-keys so len(keys)==kv_committed_len.
        if self.is_pipo:
            all_tokens = req.origin_input_ids + req.output_ids
            token_count = min(kv_committed_len * 2, len(all_tokens))
            # Ensure even length for pair-key construction
            token_count = token_count // 2 * 2
            token_ids = all_tokens[:token_count]
        else:
        ...
        # Convert to appropriate key format
        if self.is_eagle:
            keys = convert_to_bigram_key(token_ids)
        elif self.is_pipo:
            keys = convert_to_pair_key(token_ids)
        else:
            keys = token_ids
    ...
    def cache_unfinished_req(self, req: Req, chunked=False):
        ...
        # ── PIPO: fill_ids has L uncompressed tokens but req_to_token_pool
        # only has L//2 valid KV slots (compressed granularity).  Convert to
        # non-overlapping pair-keys so len(keys)==L//2, matching KV indices.
        if self.is_pipo:
            kv_len = len(token_ids) // 2  # compressed KV count
            kv_indices = self.req_to_token_pool.req_to_token[
