...
def convert_to_pair_key(tokens: List[int]) -> List[Tuple[int, int]]:
    """Convert token IDs to pair-keys for PIPO RadixCache compatibility.

    PIPO compresses every consecutive pair of tokens into a single latent
    vector, so the KV cache has half as many entries as the token sequence.
    We use non-overlapping pairs as RadixCache keys to maintain a 1:1 mapping
    between keys and KV slots.

    [t0, t1, t2, t3, t4, t5] -> [(t0,t1), (t2,t3), (t4,t5)]

    The input length MUST be even (guaranteed by tokenizer-level padding).
    """
    if len(tokens) and isinstance(tokens[0], tuple):
        return tokens
    if len(tokens) < 2:
    ...
    assert len(tokens) % 2 == 0, (
        f"convert_to_pair_key requires even-length input, got {len(tokens)}"
    )
