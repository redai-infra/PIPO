...
class ChunkCache(BasePrefixCache):
    ...
    def cache_finished_req(self, req: Req, is_insert: bool = True):
        ...
        # PIPO compaction marks freed positions as -1 in req_to_token.
        # Filter them out to prevent freeing invalid/stale slot indices.
        if kv_indices.numel() > 0:
            valid_mask = kv_indices >= 0
            if not valid_mask.all():
                kv_indices = kv_indices[valid_mask]
        self.token_to_kv_pool_allocator.free(kv_indices)

    def cache_unfinished_req(self, req: Req, chunked=False):
        # For PIPO, kv_committed_len reflects compressed length, which
        # is the actual number of valid KV slots.  Use it instead of
        # len(fill_ids) which is at uncompressed granularity.
