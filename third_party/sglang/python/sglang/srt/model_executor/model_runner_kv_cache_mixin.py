    ...
    def _init_pools(self: ModelRunner):
        ...
        if self.server_args.attention_backend == "ascend":
            ...
            if self.is_hybrid_swa:
                ...
                # Collect full attention layer IDs from config property,
                # then merge any extra IDs registered by the model (e.g.
                # PIPO MTP block layers stored in model_config).
                if self.is_draft_worker:
                    _full_attn_ids = [0]
                else:
                    _full_attn_ids = [
                    ...
                    # Merge extra full-attention layer IDs added by
                    # PIPO or similar models.
                    extra_full = getattr(
                        self.model_config, "full_attention_layer_ids", None
                    )
                    if extra_full:
                        for lid in extra_full:
                            if lid not in _full_attn_ids:
                                _full_attn_ids.append(lid)
                self.token_to_kv_pool = HybridLinearKVPool(
                    ...
                    head_dim=self.model_config.head_dim,
                    full_attention_layer_ids=_full_attn_ids,
                    enable_kvcache_transpose=False,
                    device=self.device,
