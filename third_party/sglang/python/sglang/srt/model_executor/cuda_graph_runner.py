...
class DecodeInputBuffers(ForwardInputBuffers):
    ...
    # PIPO: token pair buffer for compression path
    input_ids_pair: Optional[torch.Tensor]
    ...
    def create(
        ...
        *,
        ...
        with torch.device(device):
            ...
            # PIPO: pre-allocate buffer for token pairs
            # [max_bs, 2].  PIPO uses both tokens for compression; MTP-no-
            # verify uses the first token for the backbone input and keeps the
            # same two-token scheduler/output contract.
            input_ids_pair = (
                torch.zeros((max_bs, 2), dtype=torch.int64)
                if enable_pipo else None
            )
    ...
    def populate_from_forward_batch(
        ...
        *,
        ...
        # PIPO: copy input_ids_pair [batch, 2]
        # NOTE: input_ids_pair has different size (raw_bs) than input_ids (raw_num_token),
        # so it must be copied separately, not in the batched foreach_copy.
        if (
            self.input_ids_pair is not None
            and forward_batch.input_ids_pair is not None
        ):
            self.input_ids_pair[:raw_bs].copy_(forward_batch.input_ids_pair)
...
class CudaGraphRunner:
    ...
    def __init__(self, model_runner: ModelRunner):
        ...
        self.enable_pipo = model_runner.server_args.enable_pipo
    ...
    def can_run(self, forward_batch: ForwardBatch):
        # PIPO Phase 2 (MTP block) must NOT use the
        # captured graph which was captured for Phase 1 (backbone).
        if getattr(forward_batch, "pipo_phase", 1) == 2:
            return False
    ...
    def capture(self) -> None:
        ...
        # PIPO: per-graph_key mapping from captured graph key →
        # backbone hidden-states tensor (a graph-internal buffer whose
        # memory is reused on every replay).
        self.pipo_backbone_hidden_buffers: Dict[Any, torch.Tensor] = {}
    ...
    def capture_one_batch_size(
        ...
        # PIPO: provide dummy input_ids_pair so that the
        # captured graph's ForwardBatch contract matches decode replay.
        if (
            self.enable_pipo and buffers.input_ids_pair is not None
        ):
            forward_batch.input_ids_pair = buffers.input_ids_pair[:bs]
        ...
        # PIPO: after capture, the Phase-1 backbone
        # hidden-state attribute holds a tensor whose GPU memory will be reused
        # (overwritten) on every graph replay. Save this reference so we can
        # hand it to the Phase-2 forward call after replay.
        pipo_backbone_hidden = getattr(
            forward_batch, "pipo_backbone_hidden", None
        )

        return graph, out, pipo_backbone_hidden
    ...
    def replay(
        ...
        # PIPO: after graph replay, the backbone hidden-
        # states buffer for this graph_key has been overwritten with fresh
        # results. Attach it to forward_batch so that the Phase-2 (MTP) forward
        # call in tp_worker can read it.
        pipo_hidden = self.pipo_backbone_hidden_buffers.get(graph_key)
        if pipo_hidden is not None:
            pipo_hidden = pipo_hidden[: self.raw_num_token]
            if self.enable_pipo:
                forward_batch.pipo_backbone_hidden = pipo_hidden
