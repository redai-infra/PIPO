...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    pipo_backbone_hidden: Optional[torch.Tensor] = None  # passed from backbone to MTP phase
    pipo_phase: int = 1  # 1=backbone (output logit1), 2=MTP (output logit2)
    pipo_sampled_token_from_phase1: Optional[torch.Tensor] = None  # token from phase1 for MTP
    # For MTP without verification: 2 tokens per decode step
    # Full padded input_ids for PIPO prefill (model embeds, then compresses pairs)
    pipo_padded_input_ids: Optional[torch.Tensor] = None
    # Per-request padded extend lengths (original, uncompressed)
    ...
    def init_new(
        ...
        ret = cls(
            ...
            input_ids_pair=batch.input_ids_pair,
            pipo_phase=batch.pipo_phase,
            pipo_padded_input_ids=batch.pipo_padded_input_ids,
            pipo_padded_extend_lens=batch.pipo_padded_extend_lens,
        )
