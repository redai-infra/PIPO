...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def initialize(self, pre_model_load_memory: float):
        ...
        # PIPO model: the MTP block has 1 extra layer beyond the backbone.
        # ``end_layer`` is already set via the model attribute above, but we
        # must also ensure that layer is registered as a full-attention layer
        # so the HybridLinearAttnBackend routes it correctly (MTP block layers
        # are always full attention).
        if self.model_config.hf_config.architectures[0] in [
            "Qwen3_5ForCausalPIPO",
        ]:
            mtp_layer_id = getattr(self.model, "_mtp_layer_id_offset", None)
            if mtp_layer_id is not None:
                if not hasattr(self.model_config, "full_attention_layer_ids") or self.model_config.full_attention_layer_ids is None:
                    self.model_config.full_attention_layer_ids = []
                self.model_config.full_attention_layer_ids.append(mtp_layer_id)
