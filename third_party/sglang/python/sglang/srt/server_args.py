...
class ServerArgs:
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--enable-pipo",
            default=ServerArgs.enable_pipo,
            action="store_true",
            help="Enable PIPO (Pair-In Pair-Out) inference mode. When enabled, the model expects input_ids_pair (2 tokens per step), runs a learned pair compressor, and outputs 2 tokens per decode step.",
        )
        parser.add_argument(
            "--pipo-conf-threshold",
            type=float,
            default=ServerArgs.pipo_conf_threshold,
        )
        parser.add_argument(
            "--revision",
            type=str,
