...
def attn_backend_wrapper(runner: "ModelRunner", full_attn_backend: "AttentionBackend"):
    ...
    if cfg := runner.mambaish_config:
        ...
        full_attn_layers = list(cfg.full_attention_layer_ids)
        # For PIPO models, the MTP block uses additional layer IDs
        # beyond the backbone that must also be routed to full attention.
        extra_full_attn = getattr(runner.model_config, "full_attention_layer_ids", None)
        if extra_full_attn:
            for lid in extra_full_attn:
                if lid not in full_attn_layers:
                    full_attn_layers.append(lid)
